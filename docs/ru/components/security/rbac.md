...
### Хранение/загрузка реестра ресурсов из БД (важно про `permissions`)

Для каталога ресурсов удобно держать отдельную таблицу:
...
('document', 'Document', 'Документы', '[1,2,4]'),
('report', 'Report', 'Отчёты', '[1]');
```

Ключевой момент: в `RbacResourceRegistry::registerDefinitions(...)` поле `permissions`
ожидается как `list<int>` (список битов), а не как единая OR-маска.

То есть:

- корректно: `permissions => [1, 2, 4]`
- некорректно: `permissions => 7`

Пример загрузки из БД:
...
Если в БД храните маску (`7`) вместо массива (`[1,2,4]`), перед регистрацией
её нужно разложить в список битов, иначе контракт `registerDefinitions` будет нарушен.

Минимальный вариант преобразования маски в список битов:
