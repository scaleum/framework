...
### Resource registry DB storage/loading (important about `permissions`)

For the resource catalog, it is convenient to keep a dedicated table:
...
('document', 'Document', 'Documents', '[1,2,4]'),
('report', 'Report', 'Reports', '[1]');
```

Key detail: in `RbacResourceRegistry::registerDefinitions(...)`, the `permissions`
field is expected as `list<int>` (list of bits), not as a single OR-mask.

So:

- correct: `permissions => [1, 2, 4]`
- incorrect: `permissions => 7`
...
If you store a mask (`7`) instead of an array (`[1,2,4]`) in DB, convert it to
a bit list before registration, otherwise the `registerDefinitions` contract is violated.

Minimal mask-to-bit-list conversion example:
